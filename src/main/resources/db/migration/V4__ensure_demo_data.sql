-- V4__ensure_demo_data.sql
-- This migration ensures demo data exists even if V3 was skipped
-- Uses INSERT ... ON CONFLICT to safely add data without duplicates

-- ============================================================================
-- USERS (only insert if they don't exist)
-- ============================================================================
...
-- ============================================================================
-- API KEYS (only insert if they don't exist)
-- ============================================================================
-- Main test API key: test-api-key-local-dev
...
WHERE u.email = 'test@example.com'
  AND NOT EXISTS (
    ...
    WHERE ak.key_hash = '489545ff5724037835fceb90b6533abcd4b7c23c25e58fddc6f433e43278b078'
  );
...
WHERE u.email = 'alice@example.com'
  AND NOT EXISTS (
    SELECT 1 FROM api_keys ak 
    WHERE ak.key_hash = 'basic-key-hash-' || u.id
  );
...
WHERE u.email = 'bob@example.com'
  AND NOT EXISTS (
    SELECT 1 FROM api_keys ak 
    WHERE ak.key_hash = 'standard-key-hash-' || u.id
  );

-- ============================================================================
-- PRODUCTS (only insert if they don't exist)
-- ============================================================================
...
-- ============================================================================
-- ORDERS (only insert if they don't exist)
-- ============================================================================
...
FROM users u
WHERE u.email = 'alice@example.com'
  AND NOT EXISTS (SELECT 1 FROM orders WHERE order_number = 'ORD-DEMO-001')
...
FROM users u
WHERE u.email = 'bob@example.com'
  AND NOT EXISTS (SELECT 1 FROM orders WHERE order_number = 'ORD-DEMO-002')
ON CONFLICT (order_number) DO NOTHING;
...
WHERE u.email = 'charlie@example.com'
  AND NOT EXISTS (SELECT 1 FROM orders WHERE order_number = 'ORD-DEMO-003')
ON CONFLICT (order_number) DO NOTHING;
...
WHERE u.email = 'diana@example.com'
  AND NOT EXISTS (SELECT 1 FROM orders WHERE order_number = 'ORD-DEMO-004')
ON CONFLICT (order_number) DO NOTHING;
...
FROM users u
WHERE u.email = 'alice@example.com'
  AND NOT EXISTS (SELECT 1 FROM orders WHERE order_number = 'ORD-DEMO-005')
ON CONFLICT (order_number) DO NOTHING;

-- ============================================================================
-- ORDER ITEMS
-- ============================================================================
-- Add items to ORD-DEMO-002
